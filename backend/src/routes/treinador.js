...
router.patch('/treinador', auth, async (req, res) => {
    try {
        const pokemon = await searchByKeyAndUpdate(req.body, 'Treinador', ['cpf'], [req.user.cpf], queryFindByCpf, ['cpts', 'salario_base', 'instituto'])
        res.send(pokemon)
...
router.delete('/treinador', auth, async (req, res) => {
    ...
    try {
        ...
        res.send({ msg: 'Treinador apagado com sucesso' })
