...
const { queryInsert, queryFindByCpf, queryDeleteByCpf, queryDeleteByCpfAndProf, queryFindByCpfAndProf } = require('../models/proficiencia')
...
router.delete('/proficiencia/all', auth, async (req, res) => {
    ...
    try {
        ...
        res.send({ msg: 'Proficiencias deletadas com sucesso' })
    } catch (e) {
        res.status(500).send(e)
    }
})
...
router.delete('/proficiencia', auth, async (req, res) => {
    ...
    try {
        ...
        res.send({ msg: 'Proficiencia deletada com sucesso' })
    } catch (e) {
        res.status(500).send(e)
    }
})
