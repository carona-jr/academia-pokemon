...
router.patch('/aprimora', auth, async (req, res) => {
    ...
    try {
        ...
        [codigo, cpf, hora_de_entrada], queryFindByCodigoDeptAndHour, ['codigo_pokemon', 'cpf', 'hora_de_saida'], ['codigo_pokemon'])
...
router.delete('/aprimora/all', auth, async(req,res)=>{
    ...
    try{
        ...
        res.send({'msg': 'Aprimoramento deletado com sucesso' })
    }catch(e){
        res.status(500).send(e)
    }
})
