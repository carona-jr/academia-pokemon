...
const searchByKeyAndUpdate = (data, key, keyword, key2, keyword2, objValues, allowedUpdates = [], arrInt = []) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            objValues.values = [keyword, keyword2]
            ...
            let createNewQuery = 'UPDATE Pokemon SET '
            ...
            if(!arrInt.includes(key))
                queryToUpdate += ` AND ${key2} = '${keyword2}'`
            else {
                queryToUpdate += ` AND ${key2} = ${keyword2}`
            }
